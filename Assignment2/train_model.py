...
class DependencyModel(Module): 
  ...
  def __init__(self, word_types, outputs):
    ...
    #hidden layer linear
    self.hidden_layer = Linear(inpt_n * self.embedding_dim, hidden_n)
    #output layer linear
    self.output_layer = Linear(hidden_n, out_n)
  ...
  def forward(self, inputs):
    ...
    flattened_embeddings = embedded_inputs.view(embedded_inputs.size(0), -1)

    hidden_output = relu(self.hidden_layer(flattened_embeddings))
