...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        input_idx = []
        #get index 
        def get_word_index(word, pos_tag):
            if word is None:
            ...
            elif not self.word_vocab.get(word.lower()):
                return self.word_vocab['<UNK>']
            else:
                return self.word_vocab[word.lower()]
            
        #process top 3 elements
        def process(elements, is_stack=True):
            for i in range(1, 4):
                if len(elements) >= i:
                    word = words[elements[-i]]
                    pos_tag = pos[elements[-i]]
                    input_idx.append(get_word_index(word, pos_tag))
                else:
                    input_idx.append(self.word_vocab['<NULL>'])
        ...
        assert len(input_idx) == 6

        return np.array(input_idx)


    def get_output_representation(self, output_pair):  
        out = np.zeros(91)
        out[self.output_labels.get(output_pair)] = 1
        return out    
