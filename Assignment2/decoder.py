...
class Parser(object):
    ...
    def parse_sentence(self, words, pos):
        ...
        while state.buffer:
            feats = self.extractor.get_input_representation(words, pos, state)
            preds = self.model(torch.tensor(feats).unsqueeze(0))
            probability = preds.squeeze(0).detach().numpy()

            indxs = np.argsort(probability)[::-1]
            ...
            for i in indxs:
                ...
                if key == 'shift':
                    ...
                    if val != 'root' and len(state.stack) > 0:
                        state.left_arc(val)
                        break
